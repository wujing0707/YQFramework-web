...
$.extend($.fn.datagrid.methods, {
	...
	function getData(){	
		...
		$("#dg").datagrid({
							...
							toolbar : '#tb',
							iconCls : 'icon-user',
							url : '<%=basePath%>creditArchives/getFocusGroupsInfoList.action?zylx='+zylx,
							loadMsg : "载入中...请等待.",
...
//	});
	...
	function showDetail(id) {
		...
		window.open('<%=basePath%>creditArchives/getFocusGroupsDetail.action?sfzh='+id+'&zylx='+zylx);
		//window.location.href = '<%=basePath%>creditArchives/getFocusGroupsDetail.action?sfzh='+id+'&zylx='+zylx;
		}
	
	
	function showQueryTip(id) {
				$.ajax({
					url:"<%=basePath%>creditArchives/getFocusGroupsDetail.action?ID="+id,
	...
	function conditionSearch(){
		...
		zylx=encodeURI(encodeURI(zylx,'utf-8'),'uft-8');
		name=encodeURI(encodeURI(name,'utf-8'),'uft-8');
		var urlStr='<%=basePath%>creditArchives/getFocusGroupsInfoList.action?name='+name+'&sfzhm='+idNo+'&zylx='+zylx;
