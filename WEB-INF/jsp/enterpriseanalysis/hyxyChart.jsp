...
$(function(){
	...
	function getEnterpriseChat(){
		...
		$("#detailList").hide();
		$("#searchForList").hide();
		var hylxCode=$("#hylx").combobox('getValues');
		var timeBegin=$("#timeBegin").val();
		var timeEnd=$("#timeEnd").val();
		hylxCode=encodeURI(encodeURI(hylxCode, "UTF-8"), "UTF-8");
		var enterpriseUrl="<%=basePath%>enterpriseanalysis/hyxy.action?hylxCode="+hylxCode+"&timeBegin="+timeBegin+"&timeEnd="+timeEnd;
		$.ajax({
                url: enterpriseUrl,
                dataType: 'json',
                beforeSend:ajaxLoading,//发送请求前打开进度条
            ...
            if(yibanCount==0&&jiaozhongCount==0&&yanzhongCount==0){
			 ...
			 $('#container').highcharts({ 
				 ...
				 title: { text: '各行业失信企业统计' },
				  xAxis: {
					   categories: ret.bar.hylx 
					}, 
			 ...
			 $('#container2').highcharts({ 
   			...
   			//列表
			  var htmlStr = "<tr style='background-color:#81AACF'><td height='40px'>序号</td><td>行业</td>"+
              "<td>严重</td><td>较重</td><td>一般</td><td>合计</td></tr>";
          $.each(ret.list,function(i,d){
              htmlStr=htmlStr+"<tr><td height='30px'>"+(i+1)+"</td><td>"+d.行业+"</td><td><a href='#' onclick=\"showDetailList('"+d.行业+"','严重')\"  style=\"text-decoration:underline;color:blue\">"+d.严重+"</a></td>"+
              "<td><a href='#' onclick=\"showDetailList('"+d.行业+"','较重')\" style=\"text-decoration:underline;color:blue\">"+d.较重+"</td><td><a href='#' onclick=\"showDetailList('"+d.行业+"','一般')\" style=\"text-decoration:underline;color:blue\">"+d.一般+"</a></td><td>"+
              "<a href='#' onclick=\"showDetailList('"+d.行业+"','合计')\" style=\"text-decoration:underline;color:blue\">"+d.总计+"</a></td></tr>"
          });
          var enterTable = "<span><h1>行业失信等级统计表</h1></span><br/><table border='1' style='width:900px;text-align:center;' align='center'>"+htmlStr+"</table>";
	...
	function showPieChart(hylxCode){
		var timeBegin=$("#timeBegin").val();
		var timeEnd=$("#timeEnd").val();
			 ...
			 $('#container2').highcharts({ 
				 chart: { plotBackgroundColor: null,
				  plotBorderWidth: null,
				   ...
				   title: { text: '失信等级占比图' }, 
				   tooltip: { pointFormat: '{series.name}: <b>{point.percentage:.1f}%</b>' },
				    plotOptions: { pie: { size:'75%', allowPointSelect: true, cursor: 'pointer', 
					    dataLabels: { enabled: false, color: '#000000',
					     connectorColor: '#000000',
					      format: '<b>{point.name}</b>: {point.percentage:.1f} %' },
					      showInLegend: true //是否显示图例
	...
	function showDetailList(hylx,sxdj){
		  $.ajax({
	             type: "GET",
	             url: "<%=basePath%>enterpriseanalysis/getSxqyList.action",
	             data: {hylx:hylx,sxdj:sxdj,timeBegin:$("#timeBegin").val(),timeEnd:$("#timeEnd").val() },
	             dataType: "json",
	             beforeSend:ajaxLoading,//发送请求前打开进度条
	             success: function(data){
	                         $('#detailList').empty();   //清空detailList里面的所有内容
	                         var html = "<tr style='background-color:#81AACF'><td height='40px'>序号</td><td>失信等级</td>"+
	                         "<td>企业名称</td><td>组织机构代码</td><td>工商注册号</td><td style='width:15%'>失信时间</td></tr>";
	...
	function exportExcel(){
        ...
        var hylx=  $('#hylxForExport').val();
		var timeBegin=$("#timeBegin").val();
		var timeEnd=$("#timeEnd").val();
		//hylxCode=encodeURI(encodeURI(hylxCode, "UTF-8"), "UTF-8");
		window.location.href="<%=basePath%>enterpriseanalysis/hyxyExport.action?hylx="+hylx+"&sxdj="+sxdj+"&timeBegin="+timeBegin+"&timeEnd="+timeEnd;
	}
...
<div class="place">
	...
	<div id="tb" style="padding:5px;height:auto">
				<label for="dept">请选择行业:</label> <input 
					class="easyui-combobox" id="hylx" editable="false"
	...
	<font color="red" size="5">&nbsp;&nbsp;&nbsp;此查询条件无统计结果，请重新选择条件后查询，谢谢！</font>
	 	...
	 	<input type="hidden" id="sxdjForExport"/>
  		 <input type="hidden" id="hylxForExport"/> 
  <div id="container" style="width:55%;height:550px;float:left;padding-right:40px;"></div>
  <div id="container2" style="width:35%;height:500px;float:left;"></div>
