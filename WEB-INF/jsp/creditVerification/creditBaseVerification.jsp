				...
				.datagrid(
						{
							...
							toolbar : '#tb',
							iconCls : 'icon-user',
							url : "<%=basePath%>creditVerification/getCreditBaseVerificationList.action",
							loadMsg : "载入中...请等待.",
...
function cvInfoSearch(){
   ...
   var check = '${check}';
   if (check == 'Y') {
	  window.open("<%=basePath%>creditVerification/getDetailVerificationInfo.action?checkcode=" + checkcode, 'newwindow','height=900,width=1000,top='+(window.screen.availHeight-30-900)/2+
	            ',left='+(window.screen.availWidth-10-1000)/2+
	            ',toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no');
	} else {
	  window.location.href = "<%=basePath%>creditVerification/getDetailVerificationInfo.action?checkcode=" + checkcode;
	}
...
//自查
function selfSearch(){
	var zzjgdm = document.getElementById("idcode").value;
	var check = '${check}';
	if (check == 'Y') {
	  window.open("<%=basePath%>creditVerification/getSelfDetailVerificationInfo.action?zzjgdm="+zzjgdm, 'newwindow','height=900,width=1000,top='+(window.screen.availHeight-30-900)/2+
	            ',left='+(window.screen.availWidth-10-1000)/2+
	            ',toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no');
	} else {
	  window.location.href = "<%=basePath%>creditVerification/getSelfDetailVerificationInfo.action?zzjgdm="+zzjgdm;
	}
 ...
 function createCheckCode(){
    $.ajax({
	    type:"post",
	    url:"<%=basePath%>creditVerification/toCreateCheckCode.action",
	    async:false,
	   	data:{},
	   	dataType:"json",
	   		...
	   		if(data != null){
			    $("#dg").datagrid('reload');
	   			$.messager.confirm("提示","信用核查码生成成功，确定保存吗？", function(r){
				    if(r){
			   			var qymc = data.qymc;
			   			qymc=encodeURI(qymc,"UTF-8");
			   			qymc=encodeURI(qymc,"UTF-8");
			   			var gszch = data.gszch;
			   			gszch=encodeURI(gszch,"UTF-8");
			   			gszch=encodeURI(gszch,"UTF-8");
			   			var ccode = data.ccode;
			   			var fddbr = data.fddbr;
			   			fddbr=encodeURI(fddbr,"UTF-8");
			   			fddbr=encodeURI(fddbr,"UTF-8");
			   			var creattime2 = data.creattime2;
			   			var validtime = data.validtime;
			   			window.location.href = "<%=basePath%>creditVerification/toCreatePDF.action?qymc="+qymc+"&gszch="+gszch+"&ccode="+ccode+"&fddbr="+fddbr+"&creattime2="+creattime2+"&validtime="+validtime;
			   		}
		   		});
	        } else{
	            return false;
...
<body style="height:450px" onload="showError();">
	<div class="padT23">
		<div id="tb" style="padding:5px;height:auto;">
				<form id="ccform" name="ccform" action="<%=basePath%>creditVerification/getDetailVerificationInfo.action" method="post">
				<input type="hidden" id="idcode" value="${idnumber }">
				<input type="hidden" id="Msg" value="${Msg }">
				<!--  核查查询码： <input id="checkcode" name ="checkcode" type="text" class="easyui-validatebox" data-options="required:true" 
					style="width: 100px">&nbsp; 
					<a href="#"
					class="easyui-linkbutton" iconCls="icon-search"
					onclick="cvInfoSearch();">查询企业信用报告</a>&nbsp;&nbsp;
					<a href="#"
